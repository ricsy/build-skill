...
def test_copy_rule_dir_to_parent(tmp_path):
    ...
    cfg = PackConfig(file_copy_rules=[FileCopyRule(from_="src", to="scripts", glob=None)])
    ...
    assert "test-skill/scripts/a.txt" in names


def test_copy_rule_dir_no_to(tmp_path):
    """目录复制：from 无 to（直接复制到根目录）"""
    skill_dir = tmp_path / "skills" / "test-skill"
    skill_dir.mkdir(parents=True)
    (tmp_path / "src").mkdir()
    ...
    cfg = PackConfig(file_copy_rules=[FileCopyRule(from_="src", to=None, glob=None)])
    packer = TarPacker(cfg)
    result = packer.pack("test-skill", str(tmp_path / "dist"), str(tmp_path / "skills"))

    with tarfile.open(result.output_file) as tf:
        names = [m.name for m in tf.getmembers()]
    assert "test-skill/src/b.txt" in names
...
def test_copy_rule_dir_with_glob(tmp_path):
    ...
    cfg = PackConfig(file_copy_rules=[FileCopyRule(from_="src", to="scripts", glob="*.py")])
    packer = TarPacker(cfg)
    result = packer.pack("test-skill", str(tmp_path / "dist"), str(tmp_path / "skills"))
    ...
    with tarfile.open(result.output_file) as tf:
        names = [m.name for m in tf.getmembers()]
    # glob="*.py" 只复制 .py 文件
    assert "test-skill/scripts/a.py" in names
    assert "test-skill/scripts/c.py" in names
    assert "test-skill/scripts/b.txt" not in names
...
def test_copy_rule_file(tmp_path):
    ...
    cfg = PackConfig(file_copy_rules=[FileCopyRule(from_="config.yaml", to=None, glob=None)])
    packer = TarPacker(cfg)
    result = packer.pack("test-skill", str(tmp_path / "dist"), str(tmp_path / "skills"))

    with tarfile.open(result.output_file) as tf:
        names = [m.name for m in tf.getmembers()]
    assert "test-skill/config.yaml" in names
...
def test_pack_excludes_pycache(tmp_path):
    ...
    cfg = PackConfig(
        file_copy_rules=[FileCopyRule(from_="src", to="scripts", glob=None)],
    ...
    # 源码文件应保留
    assert "test-skill/scripts/main.py" in names
